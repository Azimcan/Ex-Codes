...
def an_counter(L, n):
	return (2/L)*quad(lambda x: (7*x**2+5*x+1)*math.cos((2*math.pi*n*x)/L), -L/2, L/2)[0]

def bn_counter(L, n):
	return (2/L)*quad(lambda x: (7*x**2+5*x+1)*math.sin((2*math.pi*n*x)/L), -L/2, L/2)[0]

def a0_counter(L):
	return (1/L)*quad(lambda x: (7*x**2+5*x+1), -L/2, L/2)[0]
